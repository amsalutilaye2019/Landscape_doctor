# ------------------------------------------------------------------------------
# Script to perform the forward process
# ------------------------------------------------------------------------------
...
rothC_Forward <-
  function(pts_path,
    ...
    setwd(warmup_path)
    message(noquote("Reading warmup points ..."))
    WARM_UP <- readOGR(warmup_aoi)
    
    # OPEN THE STACK WITH THE VARIABLES FOR THE FOWARD PROCESS
    setwd(forward_stack_path)
    message(noquote("Reading forward stack ..."))
    Stack_Set_1 <- stack(forward_stack)
