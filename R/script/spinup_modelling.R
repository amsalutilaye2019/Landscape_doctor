...
rothc_spinup <- function(pts_path, spinup_stack_path, years = 500, output_path){
    rm(list=ls()) 
  ...
  Vector <- readOGR("eth_points.shp")
    ...
    if (any(is.na(Evp[, 2])) |
      ...
      bare1 <-
        (Cov1 > 0.8)           # If the surface is bare or vegetated
      LU <- LU_im[i]
    ...
    # SOC POOLS AFTER number of years RUN WITH C INPUT EQUILIBRIUM
    if (LU==2) {
      RPM_p_2 <-
